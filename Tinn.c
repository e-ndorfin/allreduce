 ...
 * training. All code from the top of the file to xtprint() is from
 * the original library.
 * The functions below xtprint() (xtforward, xtbackward, xtapply,
 * xtgetweights, xtsetweights) were added by us for distributed training.
 */
...
// Public forward pass.
void xtforward(const Tinn t, const float *const in)
{
    fprop(t, in);
}

// Computes gradients into grad buffer without modifying weights.
// grad layout: [nw weight gradients][nb bias gradients]
// Returns total error (0.5 * MSE)
float xtbackward(const Tinn t, const float *const in, const float *const tg, float rate, float *grad)
{
    float *grad_x = grad + t.nhid * t.nips;
    for (int i = 0; i < t.nhid; i++)
    {
        float sum = 0.0f;
        for (int j = 0; j < t.nops; j++)
        {
            ...
            const float b = pdact(t.o[j]);
            sum += a * b * t.x[j * t.nhid + i];
            grad_x[j * t.nhid + i] = rate * a * b * t.h[i];
        }
        for (int j = 0; j < t.nips; j++)
            grad[i * t.nips + j] = rate * sum * pdact(t.h[i]) * in[j];
    }
    // bprop doesn't update biases => bias gradients are zero.
    grad[t.nw] = 0.0f;
    grad[t.nw + 1] = 0.0f;
    return toterr(tg, t.o, t.nops);
}

// Applies gradient vector to weights and biases (subtracts).
void xtapply(const Tinn t, const float *grad)
{
    for (int i = 0; i < t.nw; i++)
        t.w[i] -= grad[i];
    for (int i = 0; i < t.nb; i++)
...
// Copies weights and biases into a flat buffer.
void xtgetweights(const Tinn t, float *buf)
{
    memcpy(buf, t.w, t.nw * sizeof(float));
    memcpy(buf + t.nw, t.b, t.nb * sizeof(float));
}

// Loads weights and biases from a flat buffer.
void xtsetweights(const Tinn t, const float *buf)
{
    memcpy(t.w, buf, t.nw * sizeof(float));
    memcpy(t.b, buf + t.nw, t.nb * sizeof(float));
}
